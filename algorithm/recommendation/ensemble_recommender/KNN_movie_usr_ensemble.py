...
# 首先用KNN对输入的用户进行相似度匹配然后挑选出最接近的10个其他用户
...
class KNN_ensemble:
    def __init__(self, mode=0):
        ...
        self.index = pd.read_csv(os.path.join(DATA_DIR, 'personal', 'movies.csv'))
        self.reader = Reader()
        self.ratings = pd.read_csv(os.path.join(DATA_DIR, 'personal', 'ratings.csv'))
